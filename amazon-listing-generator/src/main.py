...
class AmazonListingGenerator:
    ...
    def generate_listing(self):
        ...
        try:
            ...
            # 1. 提取对标链接数据
            self.root.after(0, lambda: self.progress['value'] = 10)
            self.extract_data_from_url()
            
            # 2. 生成标题
            self.root.after(0, lambda: self.progress['value'] = 30)
            en_title, cn_title = self.generate_titles()
            
            # 3. 生成五点描述
            self.root.after(0, lambda: self.progress['value'] = 50)
            en_bullets, cn_bullets = self.generate_bullets()
            
            # 4. 生成详情描述
            self.root.after(0, lambda: self.progress['value'] = 70)
